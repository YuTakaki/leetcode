class Heap {
  ...
  public heapify(nums: [number, number[]][]) {
    /**
     Procedures:
  ...
  public pop() {
    ...
    this.heap[1] = this.heap.pop() as [number, number[]];
    let i = 1;
  ...
  public porcalate_down(i: number) {
    ...
    while (i * 2 < this.heap.length) {
      if (
        ...
        this.heap[Math.floor(i * 2 + 1)][0] < this.heap[Math.floor(i * 2)][0] &&
        this.heap[i][0] > this.heap[Math.floor(i * 2 + 1)][0]
      ) {
        const temp = this.heap[Math.floor(i * 2 + 1)];
        this.heap[Math.floor(i * 2 + 1)] = this.heap[i];
        this.heap[i] = temp;
        i = i * 2 + 1;
      } else if (this.heap[i][0] > this.heap[Math.floor(i * 2)][0]) {
        const temp = this.heap[Math.floor(i * 2)];
        this.heap[Math.floor(i * 2)] = this.heap[i];
        this.heap[i] = temp;
        i = i * 2;
